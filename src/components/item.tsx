...
export class Item extends React.Component<MyComponentProps, MyComponentState> {
    ...
    render() {
        return (
            <div className="item">
                <div >
                    <img className="responsive-img image-rounded" src={ this.props.character.thumbnail.path 
                        + '/standard_xlarge.' + this.props.character.thumbnail.extension }
                         alt={ this.props.character.name } title={ this.props.character.name } 
                         onClick={this.detailCharacter.bind(this)}/>
